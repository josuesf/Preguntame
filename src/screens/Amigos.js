...
export default class Amigos extends Component {
    ...
    recuperarAmigos = () => {
        fetchData('/ws/get_all_users', 'POST', {}, (res, err) => {
            if (err)
                alert(err)
            this.setState({ usuarios: res.filter(u=>u.usuario!=this.state.usuario_propietario) })
            console.log(res)
        })
    }
    render() {
                        ...
                        {this.state.usuarios.map(u =>
                            <View key={u.usuario}  activeOpacity={0.6} style={{ paddingVertical: 10 }}>
                                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                    ...
                                    <TouchableOpacity onPress={()=>replace('chat',{usuario:u.usuario,usuario_propietario:this.state.usuario_propietario})} style={{ borderRadius: 5, backgroundColor: '#02C39A', alignItems: 'center' }}>
